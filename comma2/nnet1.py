...
#assert(dim == 12 * 16 * 32)
#print(dim)
	weights = _variable_with_weight_decay('weights', shape=[6144, 384], stddev=0.04, wd=0.004)
...
#	_activation_summary(local4)
	...
	local4 = local3
	weights = _variable_with_weight_decay('weights', shape=[384, 2], stddev=0.04, wd=0.0)
    ...
    return tf.square(linear_end[:,0])/(tf.square(linear_end[:,1]) + epsilon)

def loss(ans, labels):
    tf.add_to_collection('losses', tf.reduce_mean(tf.squared_difference(ans, labels)/(tf.square(labels) + 1e-4)))
